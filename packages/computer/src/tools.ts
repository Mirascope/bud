...
export const computerTool = LLM.defineTool({
  ...
  description: [
    ...
    "- `computer bash '<command>' [--cwd <path>] [--shell <shell>]` - run one command and return its output.",
    "- `computer terminal-start|terminal-write|terminal-read|terminal-kill` - manage an interactive terminal session.",
  ].join("\n"),
