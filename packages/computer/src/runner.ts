import { computerCommand } from "./cli.ts";
...
const makeBufferedTerminalLayer = (buffer: Ref.Ref<string[]>) =>
  Layer.succeed(Terminal.Terminal, {
    ...
    readInput: Effect.die(
      new Error("Terminal.readInput unavailable in computer runner"),
    ),
    readLine: Effect.die(
      new Error("Terminal.readLine unavailable in computer runner"),
    ),
    display: (text: string) =>
      Ref.update(buffer, (lines) => [...lines, text]).pipe(Effect.asVoid),
  } as unknown as Terminal.Terminal);

const cli = Command.run(computerCommand, {
  name: "computer",
  version: "0.0.0",
});
...
export function runComputerCli(
  command: string,
): Effect.Effect<string, never, Computer> {
