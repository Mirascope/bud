...
export const streamHostedProvider = createServerFn({ method: "POST" })
  .inputValidator(validateHostedProviderStreamInput)
  .handler(({ data }) => {
    const provider = makeHostedProvider(data.provider);
    const encoder = new TextEncoder();

    const body = new ReadableStream<Uint8Array>({
      start(controller) {
        void Effect.runPromise(
          Stream.runForEach(provider.stream(data.args), (chunk) =>
            Effect.sync(() => {
          ...
          .catch((error: unknown) => {
            const providerError = toProviderError(error, data.provider);
            controller.enqueue(
              encoder.encode(
                encodeSse("error", {
...
export const getHostedProviderAvailability = createServerFn({
  method: "GET",
}).handler((): HostedProviderAvailability => {
  return {
